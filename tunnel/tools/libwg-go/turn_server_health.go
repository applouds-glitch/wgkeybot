...
//
// assignServers puts every stream on one TURN server, and the rest of the list
// acts as failover for a single attempt only. That is deliberate — a stream
// returns to the session's server on the next reconnect instead of sticking to
// whichever host won a race — but on its own it left no way out of a server that
// is simply down: the stream kept going back to it forever, and the only thing
// that grew was its reconnect backoff.
//
